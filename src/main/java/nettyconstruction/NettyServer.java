...
import io.SelectorExample;

/**
 * @author zhailz
 *
 * @version 2018年3月9日 下午3:35:53
 */
...
//		servChannel.register(selector2, SelectionKey.OP_READ);
//		servChannel.register(selector, SelectionKey.OP_WRITE, new SelectorExample());
					...
					try {
						if (key.isValid()) {
							...
							if (key.isReadable()) {
								...
								if (readBytes > 0) {
									// 切换读写模式 详见下面的图, 表示自己目前可以读 [position,
									// limit]
									readBuffer.flip();
